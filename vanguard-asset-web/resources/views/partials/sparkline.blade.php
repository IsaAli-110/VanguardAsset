{{-- Sparkline SVG partial --}}
{{-- Expects: $data (array of numbers), $color (hex color) --}}
@php
    $max = max($data) ?: 1;
    $min = min($data);
    $range = $max - $min ?: 1;
...
<div class="sparkline-container">
    <svg viewBox="0 0 {{ $width }} {{ $height }}" preserveAspectRatio="none">
        <defs>
            <linearGradient id="sparkGrad{{ md5(implode(',', $data)) }}" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="{{ $color }}" stop-opacity="0.4"/>
                <stop offset="100%" stop-color="{{ $color }}" stop-opacity="0"/>
            </linearGradient>
        </defs>
        <path d="{{ $areaPath }}" fill="url(#sparkGrad{{ md5(implode(',', $data)) }})"/>
